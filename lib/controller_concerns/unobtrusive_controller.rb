module UnobtrusiveController
  ...
  module ClassMethods
    ...
    def unobtrusive(options)
      helper_method :collection, :resource, :resource_class, :resource_collection_name, :resource_name, :collection_url, :collection_route, :resource_url, :resource_route, :new_resource_url
      ...
      unobtrusive_method :collection_url do
        url_for(collection_route)
      end

      unobtrusive_method :collection_route do
        [relationship_name]
      end

      unobtrusive_method :resource_url do
        url_for(resource_route)
      end

      unobtrusive_method :resource_route do
        [resource]
      end

      unobtrusive_method :new_resource_url do
        url_for(new_resource_route)
      end

      unobtrusive_method :new_resource_route do
        [:new, relationship_singular_name]
      end
      ...
      if method_defined?(:parent_class)
        ...
        unobtrusive_method :parent_url do
          url_for(parent_route)
        end

        unobtrusive_method :parent_route do
          [parent]
        end
